...
def mera_indices(L, kernel_size, repeat):
    index_list = []
    depth = int(log(L / kernel_size, 2) + 1)
    for i in range(depth):
        for _ in range(repeat):
            index_list.append(
                get_indices([L, L], kernel_size, kernel_size,
                            kernel_size * 2**i, 2**i, 0))
            index_list.append(
                get_indices([L, L], kernel_size, kernel_size,
                            kernel_size * 2**i, 2**i, kernel_size * 2**i // 2))
...
class MERA(HierarchyBijector):
    def __init__(self, layers, L, kernel_size, repeat, prior=None):
        indexI, indexJ = mera_indices(L, kernel_size, repeat)
